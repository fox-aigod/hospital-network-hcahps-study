...
ONC_REQUIRED = {
    "aha_id", "mstate", "mcrnum", "year", "national_network",
    "vendor_network", "hio", "tefca_part", "tefca_plan",
}
CMS_REQUIRED = {
...
def prepare_onc(path: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for column in ("hio", "national_network", "vendor_network", "tefca_part", "tefca_plan"):
        frame[column] = frame[column].map(parse_binary).astype("int8")

    missing_ccn = int(frame["ccn"].eq("").sum())
    usable = frame.loc[frame["ccn"].ne("")].copy()
    duplicate_ccns = sorted(usable.loc[usable.duplicated("ccn", keep=False), "ccn"].unique())

    # The source represents each hospital by its most recent available survey.
    # Sort order makes the one observed cross-year duplicate resolution explicit
    # and deterministic; AHA ID is a final tie-breaker only.
...
def prepare_cms(path: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    excluded_non_ccn_rows = int(frame["ccn"].eq("").sum())
    frame = frame.loc[frame["ccn"].ne("")].copy()

    # Any true numeric-CCN duplicates must be resolved deterministically rather
    # than inheriting source-file order. The archived snapshot has none.
    frame = frame.sort_values(
        ["ccn", "State", "Facility Name"],
        ascending=[True, True, True],
        kind="mergesort",
    )
    duplicates = sorted(frame.loc[frame.duplicated("ccn", keep=False), "ccn"].unique())
    unique = frame.drop_duplicates("ccn", keep="first")
    audit = {
        "raw_rows": int(len(raw)),
...
def build_stage1_cohort(root: Path) -> dict[str, Any]:
    ...
    merged = onc.merge(cms, on="ccn", how="left", validate="one_to_one", indicator=True)
    ...
    locked = eligible_all_years.loc[eligible_all_years["survey_year"].isin([2024, 2025])].copy()
    ...
    locked["profile_category_code"] = locked.apply(
        lambda row: profile_category(
            int(row["hio"]), int(row["national_network"]),
            int(row["vendor_network"]), int(row["tefca_part"])
        ),
        axis=1,
    ).astype("int8")
    locked["profile_category_label"] = locked["profile_category_code"].map(PROFILE_LABELS)

    output_columns = [
        "ccn", "aha_id", "survey_year", "Facility Name", "State",
        "County/Parish", "Hospital Type", "hospital_group",
        "Hospital Ownership", "Emergency Services", "Hospital overall rating",
        "hio", "national_network", "vendor_network", "tefca_part", "tefca_plan",
        "network_count", "profile_bits", "profile_category_code",
        "profile_category_label",
    ]
    locked = locked[output_columns].rename(columns={
    ...
    matched.drop(columns="_merge").to_csv(interim_dir / "onc_cms_matched.csv", index=False)
    ...
    profile_counts = (
        locked.groupby(["profile_category_code", "profile_category_label", "hospital_group"], observed=True)
    ...
    summary = {
        ...
        "locked_2024_2025_cohort": {
            ...
            "acute_care": int((locked["hospital_group"] == "Acute Care").sum()),
            "critical_access": int((locked["hospital_group"] == "Critical Access").sum()),
